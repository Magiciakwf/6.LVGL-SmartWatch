...
static void page_stack_init(PageStack_t* stack) {
	stack->top = 0;//改变top值，将top值改为0
}

static uint8_t page_stack_push(PageStack_t* stack, Page_t* page) {
    if (stack->top >= MAX_DEPTH)//top最终值会达到MAX_DEPTH
		return -1;
    stack->pages[stack->top++] = page;//先赋值再＋＋
	return 0;
}
...
static uint8_t page_stack_pop(PageStack_t* stack) {
    if (stack->top <= 0)
		return -1;
    stack->pages[--stack->top]->deinit();//先减后赋值，重点是调用deinit
    return 0;
}
...
void Page_Back(void) {
    ...
    if (page_stack_is_empty(&PageStack)) {
        // 如果栈为空，入栈并切换到MenuPage
			page_stack_push(&PageStack, &Page_Home);//如果栈为空，回到主页去
        page_stack_push(&PageStack, &Page_Menu);
        Page_Menu.init();
        lv_scr_load_anim(*Page_Menu.page_obj, LV_SCR_LOAD_ANIM_MOVE_RIGHT, 100, 0, true);
			//lvgl屏幕画面切换函数，*Page_Menu.page_obj表示要切换到的画面，第二个表示动画模式
			//100表示动画持续时间，0表示开始前的延迟时间，true表示指定是在切换到新屏幕后删除旧屏幕
    } else {
        // 切换到上一个页面
        Page_t *previous_page = PageStack.pages[PageStack.top - 1];
        previous_page->init();//先初始化后加载
