...
struct
{
	...
	/* Optional environment/health payload fields are disabled. */
	uint16_t stepNum;
}BLEMessage;
...
static uint8_t TimeDate_IsValid(const RTC_DateTypeDef *date, const RTC_TimeTypeDef *time)
{
	static const uint8_t days_in_month[12] =
		{31U, 28U, 31U, 30U, 31U, 30U, 31U, 31U, 30U, 31U, 30U, 31U};
	uint8_t max_day;

	if(date->Year == 0U || date->Year >= 99U || date->Month == 0U || date->Month > 12U)
		return 0U;
	max_day = days_in_month[date->Month - 1U];
	if(date->Month == 2U && (date->Year & 3U) == 0U)
		max_day = 29U;

	return (uint8_t)(date->Date > 0U && date->Date <= max_day &&
...
uint8_t TimeFormat_Get(uint8_t * str)
{
	...
	if(TimeDate_IsValid(&TimeSetMessage.nowdate, &TimeSetMessage.nowtime))
	{
		...
		DataSave_Request(DATA_SAVE_EVENT_TIME_CHANGED);
		printf("TIMESETOK\r\n");
		return 0U;
	}
	...
	while(1)
	{
		if(HardInt_uart_flag)
		{
			...
			else if(!strcmp(HardInt_receive_str,"OV+SEND"))
			{
				...
				/* Only the required date, time and step data is sent over UART. */
